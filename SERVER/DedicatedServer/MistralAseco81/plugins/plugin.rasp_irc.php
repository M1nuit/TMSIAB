...
global $aseco, $linesbuffer, $ircmsgs, $outbuffer;
	$lines = $aseco->client->addCall("GetChatLines", array(50, 0));
	if (!$aseco->client->multiquery()) {
		return false;
	} else {
		$response = $aseco->client->getResponse();
		$msgs = array();
		$msgs = $response[$lines][0];
		foreach ($msgs as $msg) {
			if (!in_array($msg, $linesbuffer)) {
				if (!strstr($msg, '-IRC-')) {
					$ircmsgs[] = $msg;
				}
				if (sizeof($linesbuffer) > 100) {
	...
	if (!feof($con['socket'])) {
		...
		if (!$buffer = fgets($con['socket'], 4096)) {
			...
			if ($player != $CONFIG['nick'] && strlen($player) > 0) {
				...
				$aseco->client->addCall('ChatSendServerMessage', array($msg));
