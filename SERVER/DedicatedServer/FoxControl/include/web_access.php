...
require_once('include/urlsafebase64.php');
...
class Webaccess {
	...
	function select(&$read, &$write, &$except, $tv_sec, $tv_usec = 0) {
		...
		// if no socket to select then return immediately
	...
	private function _manageWebaccessSockets(&$receive, &$send, &$except) {
		...
		// read datas from all needed webaccess sockets
...
class WebaccessUrl {
	...
	function request(&$query) {
		...
		if (($query['Callback'] == null) || (is_array($query['Callback']) &&
			...
			if (is_string($query['QueryData']) && strlen($query['QueryData']) > 0) {
				...
				console($this->_webaccess_str . 'Bad data');
				return false;
			}
	...
	private function _open_socket($opentimeout = 0.0) {
		...
		// if socket should be closed, then close
			...
			if ($this->_wait) {
				...
				while (isset($this->_spool[0]['State']) &&
					...
					elseif ($this->_spool[0]['State'] == 'RECEIVE')
						$this->_receive($waittimeout);

					// if timeout then error
					if (microtime(true) - $this->_read_time > $waittimeout) {
						$this->_bad('Read timeout (' . $waittimeout . ')');
						break;
					}
				}
	...
	function _send($waittimeout = 20) {
		...
		do {
			...
			// if timeout then error
			if (microtime(true) - $this->_read_time > $waittimeout) {
				$this->_bad('Send timeout (' . $waittimeout . ')');
				break;
			}
	...
	function _receive($waittimeout = 20) {
		...
		if ($state === true) {
			...
			$this->_spool[0]['Times']['receive'][1] = $time - $this->_spool[0]['Times']['receive'][0];
			$this->_spool[0]['Times']['State'] = 'DONE';
	...
	function infos() {
		...
		$msg = $this->_webaccess_str
			. sprintf('[%s,%s]: %0.3f / %0.3f / %0.3f (%0.3f) / %d [%d,%d]',
			          $this->_state, $this->_spool[0]['Times']['State'],
