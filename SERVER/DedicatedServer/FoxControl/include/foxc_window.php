...
class window
{
	...
	public function code($code) //Use own code, but the foxcontrol window design
	{
		global $fc_window;
		$fc_window['Code'] = $code;
	...
	public function show($player) //display window
	{
		...
		$fc_window['PlayerButtons'] = $fc_window['Buttons'];
	...
	private function buttonPressed($data)
	{
		...
		$data[2] = $data[2] - 10000;
		$data[3] = $fc_window['PlayerButtons'][$data[2] - 1]['text'];
		if($fc_window['PlayerButtons'][$data[2] - 1]['close'] == true)
		{
		...
		for($i = 0; isset($fc_window['Content'][$i]); $i++) //For every line
		{
			...
			for($tdi = 0; isset($td[$tdi]); $tdi++) //For every td
			{
				...
				for($ci = 0; true; $ci++)
				{
					...
					if(substr($td[$tdi], $ci, 1) == '"')
					{
						if($width_began == false && $element == 0)
						{
							$width_began = true;
							$element = 1;
						}
						elseif($width_began == true) $width_began = false;
						elseif($element == 1)
						{
							$is_link = true;
							$element = 2;
						}
						elseif($is_link == true) $is_link = false;
						
					}
					elseif($width_began == true) $width = $width . substr($td[$tdi], $ci, 1);
					elseif($is_link == true) $link = $link . substr($td[$tdi], $ci, 1);
					if(substr($td[$tdi], $ci, 1) == '/') break;
				}
				$text = $td[$tdi];
				...
				$text = str_replace('id="'.$link.'"/>', '', $text);
				$text = str_replace('id="'.$link.'" ', '', $text);
				$table .= '<label posn="'.$posx.' '.$posy.' 4" sizen="'.($width - 0.5).' 2" textsize="2" text="$fff'.$text.'"/>';
