...
class XmlrpcDB{
  ...
  function XmlrpcDB($webaccess, $url, $game, $login, $password, $tool, $version, $nation, $packmask=''){
		...
		$this->_url = $url;
		$this->_server = array('Game'=>$game,'Login'=>$login,'Password'=>$password,'Tool'=>$tool,'Version'=>$version,'Nation'=>$nation,'Packmask'=>$packmask);
  ...
  function RequestWaitArray($method, $args){
		...
		if(isset($response['Message']) && is_string($response['Message'])){
			...
			if ($xmlrpc_message->parse() && $xmlrpc_message->messageType != 'fault') {
				...
				console("XmlrpcDB->RequestWaitArray() - message fault",$xmlrpc_message->message);
				$datas = array();
