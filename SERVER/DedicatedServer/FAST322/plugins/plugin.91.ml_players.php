...
function ml_playersPreloadXml($login){
	...
	$xml = '<format textsize=1 textcolor=\'0000\'></format>'
		.'<background bgcolor=\'0000\'></background><line>'
		.'<cell><icon width=0.03 height=0.03></icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_play.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_playR.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_spec.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_specforce.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_net1.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_net2.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_noplay.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_noplayR.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_nospec.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_ignore.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_kick.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_ban.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_black.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_unignore.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_unban.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_unblack.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_up.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_down.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_upG.dds</icon></cell>'
		.'<cell><icon width=0.03 height=0.03>http://slig.free.fr/img/fast_downG.dds</icon></cell>'
		.'</line>';
...
function ml_playersInitXmlStrings(){
	...
	$_ml['ml_players_head'] = '<format textsize=1 textcolor=\'ffdf\'></format>'
		.'<background bgcolor=\'111e\' bgborderx=\'0.02\' bgbordery=\'0.02\'></background>';

	$_ml['ml_players_head2'] = '<format textsize=1 textcolor=\'0000\'></format>'
		.'<background bgcolor=\'0000\' bgborderx=\'0.02\' bgbordery=\'0.02\'></background>';

	$_ml['ml_players_end'] = '<line><cell width=%f><text halign=center textsize=2 action=\'%d\'> $z$cc4%s </text></cell></line>';
	$_ml['ml_players_end2'] = '';
}


function ml_playersInitXmlPlayerInfo($isadmin,$lw,$lh,$size,$act,$num,$login,&$pl,&$pml){
	global $_ignore_list,$_ban_list,$_black_list,$_is_relay;
	$action = ($isadmin && !$_is_relay && !$pl['Relayed']) ? " action=$act" : '';
	$action .= verifyAdmin($login) ? ' textcolor=\'afaf\'' : '';
	$back = ($pml['ml_players']['Login']==$login)? " bgcolor='332e'" :'';

	$info = "<cell width=$lw$back><text halign=right$action>".$pl['Login'].'  </text></cell>';
	
	return $info;
}


function ml_playersInitXmlPlayerImg($isadmin,$lw,$lh,$size,$act,$num,$login,&$pl,&$pml){
	...
	$ln = 0.024;
	if($pl['Relayed'])
		$info = "<cell width=$ln textcolor='c6f7'><text halign=center>\$z".($num%100)."</text></cell>";
	else
		$info = "<cell width=$ln textcolor='6f07'><text halign=center>\$z".($num%100)."</text></cell>";
	...
	//$url1 = null; $url2 = null; 
	if($url1)
		$info .= "<cell width=$lh><icon$center width=$size height=$size>$url1</icon></cell>";
	else
		$info .= "<cell width=$lh><text>\$z</text></cell>";
	if($url2)
		$info .= "<cell width=$lh><icon$center width=$size height=$size>$url2</icon></cell>";
	else
		$info .= "<cell width=$lh><text>\$z</text></cell>";

	if(isset($_ignore_list[$login])){
		$info .= "<cell width=$lh><icon $center width=$size height=$size>http://slig.free.fr/img/fast_ignore.dds</icon></cell>";
		$nimg++;
	}
	if(isset($_ban_list[$login])){
		$info .= "<cell width=$lh><icon $center width=$size height=$size>http://slig.free.fr/img/fast_ban.dds</icon></cell>";
		$nimg++;
	}
	if(isset($_black_list[$login])){
		$info .= "<cell width=$lh><icon $center width=$size height=$size>http://slig.free.fr/img/fast_black.dds</icon></cell>";
		$nimg++;
	}
	...
	if(!isset($pml['ml_players']['Color']) || $pml['ml_players']['Color'])
		...
		$nick = $pl['NickDraw2'];
	$info .= '<cell width='.($lw-$ln-$lh*$nimg)."><text textcolor='ceff'$action>$nick</text></cell>";
...
function ml_playersUpdateXml($login,$action='show'){
  ...
  if(!is_string($login))
	...
	elseif($lmax<=56){ $size = 0.028; $cmax = 2; $lmax = 26; }
	...
	$lw += 2*$lh+0.024;
	// manialink header
	$xml = sprintf($_ml['ml_players_head'],$lw*$cmax/2);
	$xml2 = sprintf($_ml['ml_players_head2'],$lw*$cmax/2);

	// 1st line : title and other pages
	$xml .= '<line height=0.044><cell width='.($lw*$cmax).'><text halign=left textsize=2 action='
		.$_ml_act['ml_players.refresh'].'>$z$s$ff0'.localeText($login,'ml_players.title').'$z</text></cell></line>';

	$xml2 .= '<line height=0.044><cell width='.($lw*$cmax).'></cell></line>';
	...
	$xml .= "<line height=$lh>"
		.'<cell width='.(2*$lh+0.024).'></cell>'
		."<cell width=$lh><text textcolor='0000' action=$act1>  .  </text></cell>"
		.'<cell width='.($lw-4*$lh-0.024).'></cell>'
		."<cell width=$lh><text textcolor='0000' action=$act2>  .  </text></cell>"
		.'<cell width='.($lw*($cmax-1)-0.50).'></cell>'
		.'<cell width=0.50><text halign=right textsize=1 textcolor=\'cc4f\' action='.$_ml_act['ml_players.nickcolor'].'>'
		.localeText($login,$colorstr).'</text></cell></line>';
	$xml2 .= "<line height=$lh>"
		.'<cell width='.(2*$lh+0.024).'><text>.</text></cell>'
		."<cell width=$lh><icon width=$size2 height=$size2>$url1</icon></cell>"
		.'<cell width='.($lw-4*$lh-0.024).'><text>.</text></cell>'
		."<cell width=$lh><icon width=$size2 height=$size2>$url2</icon></cell>"
		//.'<cell width='.($lw*($cmax-1)).'></cell></line>';
		.'</line>';

	// yellow bar
	$xml .= "<line height=0.004><cell width=".($lw*$cmax).' bgcolor=\'ff08\'> </cell></line>';
	$xml2 .= "<line height=0.004><cell width=".($lw*$cmax).' bgcolor=\'0000\'> </cell></line>';

	// players list
	for($i=0;$i<$lmax; $i++){

		$xml .= "<line height=$lh>"; // width=".($lw*$cmax).'>';
		$xml2 .= "<line height=$lh>"; // width=".($lw*$cmax).'>';
		for($j=0;$j<$cmax;$j++){
			$k = $i + $j*$lmax;
			if(isset($list[$k])){
				$act = $_ml_act['ml_players.'.$k];
				$xml .= ml_playersInitXmlPlayerInfo($isadmin,$lw,$lh,$size,$act,$k+1,''.$list[$k],$_players[$list[$k]],$pml);
				$xml2 .= ml_playersInitXmlPlayerImg($isadmin,$lw,$lh,$size,$act,$k+1,''.$list[$k],$_players[$list[$k]],$pml);
			}else{
				$xml .= '<cell><text>$z</text></cell>';
				$xml2 .= '<cell><text>$z</text></cell>';
			}
		}
		$xml .= '</line>';
		$xml2 .= '</line>';
	...
	if(isset($pml['ml_players']['Login']) && isset($_players[$pml['ml_players']['Login']]) && !$_is_relay){
		...
		if(!isset($pml['ml_players']['Action']) || $pml['ml_players']['Action']==''){
			...
			$xml .= "<line height=$lh><cell width=".($lw*$cmax).'><text textsize=2>'
				.localeText($login,'ml_players.choose',$slogin,$spl['NickDraw2'],$spl['NickDraw3']).'</text></cell></line>';
			$xml2 .= "<line height=$lh><cell width=".($lw*$cmax).'></cell></line>';
			...
			$xml .= "<line height=$lh><cell width=".($lw*$cmax).'><text textsize=2>'
				...
				.'</text></cell></line>';
			$xml2 .= '<line height=$lh><cell width='.($lw*$cmax-0.30).'></cell>'
				.'<cell width=0.30><text halign=right textsize=2 textcolor=\'ff4f\' action='.$_ml_act['ml_players.confirm'].'>$z$o'
				.localeText($login,'ml.confirm').' </text></cell></line>';
		}
		...
		if($spl['Active']){
			if($spl['IsSpectator']){
				if($_GameInfos['GameMode']==2){
					$xml .= '<cell><text action='.$_ml_act['ml_players.blue'].'>$z  $s'.localeText($login,'ml_players.blue').'</text></cell>'
						."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_play.dds</icon></cell>";
					$xml .= '<cell><text action='.$_ml_act['ml_players.red'].'>$z  $s'.localeText($login,'ml_players.red').'</text></cell>'
						."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_playR.dds</icon></cell>";
				}else{
					$xml .= '<cell><text action='.$_ml_act['ml_players.play'].'>$z  $s'.localeText($login,'ml_players.play').'</text></cell>'
						."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_play.dds</icon></cell>";
				}
				if($spl['Forced']){
					$xml .= '<cell><text action='.$_ml_act['ml_players.spec'].'>$z  $s'.localeText($login,'ml_players.spec').'</text></cell>'
						."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_spec.dds</icon></cell>";
				}else{
					$xml .= '<cell><text action='.$_ml_act['ml_players.specforce'].'>$z  $s'.localeText($login,'ml_players.specforce').'</text></cell>'
						."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_specforce.dds</icon></cell>";
				}
			}else{
		...
		$xml .= '<cell><text action='.$_ml_act['ml_players.kick'].'>$z  $s'.localeText($login,'ml_players.kick').'</text></cell>'
			."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_kick.dds</icon></cell>";

		if(isset($_ignore_list[$slogin])){
			$xml .= '<cell><text action='.$_ml_act['ml_players.unignore'].'>$z  $s'.localeText($login,'ml_players.unignore').'</text></cell>'
				."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_unignore.dds</icon></cell>";
		}else{
			$xml .= '<cell><text action='.$_ml_act['ml_players.ignore'].'>$z  $s'.localeText($login,'ml_players.ignore').'</text></cell>'
				."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_ignore.dds</icon></cell>";
		}
		if(isset($_ban_list[$slogin])){
			$xml .= '<cell><text action='.$_ml_act['ml_players.unban'].'>$z  $s'.localeText($login,'ml_players.unban').'</text></cell>'
				."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_unban.dds</icon></cell>";
		}else{
			$xml .= '<cell><text action='.$_ml_act['ml_players.ban'].'>$z  $s'.localeText($login,'ml_players.ban').'</text></cell>'
				."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_ban.dds</icon></cell>";
		}
		if(isset($_black_list[$slogin])){
			$xml .= '<cell><text action='.$_ml_act['ml_players.unblack'].'>$z  $s'.localeText($login,'ml_players.unblack').'</text></cell>'
				."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_unblack.dds</icon></cell>";
		}else{
			$xml .= '<cell><text action='.$_ml_act['ml_players.black'].'>$z  $s'.localeText($login,'ml_players.black').'</text></cell>'
				."<cell width=$lh><icon width=$size height=$size>http://slig.free.fr/img/fast_black.dds</icon></cell>";
		}
		...
		$xml2 .= '<line height=$lh><cell width='.($lw*$cmax-0.30).'></cell>'
			.'<cell width=0.30><text halign=right textsize=2 textcolor=\'cc4f\' action='.$_ml_act['ml_players.quit'].'>'
			.localeText($login,'ml.quit').' </text></cell></line>';

		// manialink footer and quit
		$xml .= $_ml['ml_players_end2'];
		$xml2 .= $_ml['ml_players_end2'];

	}else{
		// manialink footer and quit
		$xml .= sprintf($_ml['ml_players_end'],$lw*$cmax,$_ml_act['ml_players.quit'],localeText($login,'ml.quit'));
		$xml2 .= $_ml['ml_players_end2'];
	}
