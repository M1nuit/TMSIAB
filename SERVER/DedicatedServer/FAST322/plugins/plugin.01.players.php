...
//
// $_fteams_rules['DrawRule'] : if there is a draw in a round, 'Average','Bestplayer','PreviousRank'. (in $_fteams_round)
//           if draw (teams sum points as same), make average score, or rank them based on bestplayers, or on previous rank
...
// $_fteams[tid]['NameDraw'] : name of team (for manialinks)  (set it using htmlspecialchars($_fteams[tid]['Name'])
...
function playersSpecialRoundRestart($text){
	...
	if($_GameInfos['GameMode'] != 0 && $_GameInfos['GameMode'] != 2 && $_GameInfos['GameMode'] != 5){
...
function playersPlayerSetup($login){
	...
	return array('Login'=>$login,
							 ...
							 'NickDraw'=>'',  // htmlspecialchars(tm_substr(stripColors($val),0,13))
							 'NickDraw2'=>'', // htmlspecialchars(tm_substr(stripColors($val),0,24));
							 'NickDraw3'=>'', // htmlspecialchars(tm_substr(stripEnlarge($val),0,24));
							 'NickDraw4'=>'', // htmlspecialchars(tm_substr($val,0,24));
							 'Nation'=>'',
							 'Path'=>'',
							 'Country'=>'',
							 'Language'=>'',
...
function playersInit($event){
  ...
  if($_memdebug>0) $_mem = memory_get_usage($_memdebugmode); else $_mem = 0;
	...
	if(!isset($_ChatFloodRateMax))
		$_ChatFloodRateMax = 4; // max flood rate within 5 seconds (max burst is double)
	if($_ChatFloodRateMax < 1.5)
		$_ChatFloodRateMax = 1.5; // 1.5 / 5s is the min
...
function playersPlayerUpdate($event,$login,$player){
	global $_debug,$_players,$_players_old,$_GameInfos,$_NumberOfChecks,$_locale_default,$_locale,$_currentTime,$_StatusCode,$_DegradedMode,$_StatusCode,$_players_checkchange;
  if(!is_string($login))
    $login = ''.$login;
		...
		if(!isset($pl[$key]) || ($pl[$key] !== $val)){
			...
			if($key == 'SpectatorStatus'){
				...
				if($pl['SpectatorStatus'] != $val){
					...
					if($_StatusCode == 4 && $_GameInfos['GameMode'] == 1 && !$pl['IsSpectator'] && $IsSpectator){ // && $pl['CheckpointNumber'] >= 0){
					...
					//if($_StatusCode == 4 && $_GameInfos['GameMode'] == 1 && $pl['IsSpectator'] && !$IsSpectator){
				...
				$val = tm_substr($val);
				if($pl['NickName'] != $val){
					$pl['NickDraw'] = htmlspecialchars(tm_substr(stripColors($val),0,13));
					$pl['NickDraw2'] = htmlspecialchars(tm_substr(stripColors($val),0,24));
					$pl['NickDraw3'] = htmlspecialchars(tm_substr(stripEnlarge($val),0,24));
					$pl['NickDraw4'] = htmlspecialchars(tm_substr($val,0,24));
				}
  ...
  if(!is_string($login))
	...
	if(isset($playerinfo['NickName'])){
		...
		if((!isset($pl['NickName']) || !isset($pl['NickDraw']) ||	$pl['NickName'] != $playerinfo['NickName'])){
			...
			$pl['NickDraw'] = htmlspecialchars(tm_substr(stripColors($playerinfo['NickName']),0,13));
			$pl['NickDraw2'] = htmlspecialchars(tm_substr(stripColors($playerinfo['NickName']),0,24));
			$pl['NickDraw3'] = htmlspecialchars(tm_substr(stripEnlarge($playerinfo['NickName']),0,24));
			$pl['NickDraw4'] = htmlspecialchars(tm_substr($playerinfo['NickName'],0,24));
		}
	}
...
function playersPlayerConnect($event,$login,$pinfo,$pdetailedinfo,$pranking){
  ...
  if(!is_string($login))
	...
	if(!$_players_firstmap && $_players_actives <= 0 && !$_WarmUp && $_FWarmUp <= 0 &&
		 ($_GameInfos['GameMode'] == 0 || $_GameInfos['GameMode'] == 2 || $_GameInfos['GameMode'] == 5)){
	...
	$pl['NickDraw'] = htmlspecialchars(tm_substr(stripColors($pl['NickName']),0,13));
	$pl['NickDraw2'] = htmlspecialchars(tm_substr(stripColors($pl['NickName']),0,24));
	$pl['NickDraw3'] = htmlspecialchars(tm_substr(stripEnlarge($pl['NickName']),0,24));
	$pl['NickDraw4'] = htmlspecialchars(tm_substr($pl['NickName'],0,24));
	...
	//if($pl['BestTime'] > 0 && $_GameInfos['GameMode'] != 3){
	...
	if(!$init && $_GameInfos['GameMode'] == 3 && $_StatusCode == 4){
...
function playersPlayerCheckpoint($event,$login,$time,$lapnum,$checkpt,$hiddenabort=false){
  ...
  if(!is_string($login))
	...
	if($_GameInfos['GameMode'] == 1 && $checkpt == 0){
	...
	$prevlap = ($_LapsDiscoFix && $_GameInfos['GameMode'] == 3) ? $pl['PrevLapNumber'] : 0;
	...
	if($checkpt+0 == 0 && $_GameInfos['GameMode'] == 3 && !$_LapsDiscoFix)
		$pl['Checkpoints'] = array();
	...
	if($_GameInfos['GameMode'] == 3){
		...
		$laptime = $time - $pl['Checkpoints'][($checkpt-$lapcheckpt-1)];
		...
		if($pl['CheckCPs'] && $_NumberOfChecks > 0 && (($checkpt+1) % $_NumberOfChecks) == 0 &&
			...
			foreach($pl['Checkpoints'] as $cp){
				if($cp < 0 || $cp <= $cptime){
					...
					console("playersPlayerCheckpoint - $login,$laptime - BAD: wrong checkpoint time $cp (".implode(',',$pl['Checkpoints']).')');
	...
	if($prevlap > 0 && $_GameInfos['GameMode'] == 3 && $lapnum >= $_GameInfos['LapsNbLaps']){
...
function playersPlayerFinish($event,$login,$time,$checkpts=null){
	...
	if($time == 0){
		...
		if($_players[$login]['FinalTime'] > 0){
			if($_debug>2) console("playersPlayerFinish({$login},{$time}):: already a final time ({$_players[$login]['FinalTime']}) -> drop event !");
			dropEvent();
			return;
		}
	}
	...
	$dbg = $time > 0 ? 0 : 3;
	if($_GameInfos['GameMode'] == 1 && $pl['RSdelays'] > 0){
	...
	$pl['FinalTime'] = $time+0;
	if($time > 0) $pl['LastCpTime'] = $time+0;
	$pl['PlayRights'] = true;


	// test for PlayerBest
	if($time > 0){
		if($_GameInfos['GameMode'] == 4){
			...
			// test if best time
			if($pl['CheckCPs'] && $time > 0 && $_GameInfos['GameMode'] != 3 && ($pl['BestTime'] <= 0 || $time < $pl['BestTime'])){
				$pl['BestTime'] = $time+0;
				$pl['BestCheckpoints'] = $checkpts;
				if(isset($pl['BestCheckpoints'][-1]))
					unset($pl['BestCheckpoints'][-1]);
	...
	if($_GameInfos['GameMode'] == 1 && $time == 0){
		$pl['CheckpointNumber'] = -1;
		$pl['Checkpoints'] = array();
		$pl['FinalTime'] = -1;
		$pl['Position'] = array('Pos'=>-1,'Check'=>-1,
	...
	$_players_checkchange = true;
	if($_GameInfos['GameMode'] != 1 && $_GameInfos['GameMode'] != 4){
		// in rounds based modes, compute players positions
		playersComputePositions();
	}
...
function playersPlayerFinish_Reverse($event,$login,$time,$checkpts=null){
	...
	if(isset($_players[$login]['NbrLapsFinished']) &&  $time > 0){
		if($_GameInfos['GameMode'] != 3 || $_players[$login]['NbrLapsFinished'] >= $_GameInfos['LapsNbLaps'])
...
function playersBeginRace($event,$GameInfos,$ChallengeInfo,$newcup,$warmup,$fwarmup){
	...
	if($_GameInfos['GameMode'] != 5 || $newcup){
	...
	if($_match_map < 0 || $_GameInfos['GameMode'] != 5 || $newcup)
...
function playersEndRace($event,$Ranking,$ChallengeInfo,$GameInfos,$continuecup,$warmup,$fwarmup){
	...
	if($_FWarmUp > 0){
		...
		$classicwud = $_NextGameInfos['GameMode'] == 5 ? $_NextGameInfos['CupWarmUpDuration'] : $_NextGameInfos['AllWarmUpDuration'];
		if($classicwud <= 0){
			// FWarmUp will start
			$_NextFWarmUp = $_FWarmUpDuration;
			$_FWarmUpState = 0;
...
function playersBeginRound($event){
	...
	if(($_GameInfos['GameMode'] != 1 && $_GameInfos['GameMode'] != 4) && $_players_round_current <= 1){
		$istmpspec = false;
		foreach($_players as $login => &$pl){
...
function playersEndRound($event,$Ranking,$ChallengeInfo,$GameInfos,$SpecialRestarting){
	...
	if($_WarmUp && !$_players_round_restarting && ($_GameInfos['GameMode'] == 0 || $_GameInfos['GameMode'] == 2 || $_GameInfos['GameMode'] == 5)){
		if($_GameInfos['GameMode'] == 5)
...
function playersEvery5seconds($event){
	...
	foreach($_players as $login => &$pl){
		$pl['ChatFloodRate'] /= 2; // reduce player chat flood rate
		if($pl['ChatFloodRateIgnore'] && $pl['ChatFloodRate'] < 2){
...
function playersComputePositions($sec=-1){
	...
	foreach($_players as $login => &$pl){
		...
		if(isset($pl['Checkpoints'][0])){
			...
			if($pl['FinalTime'] > 0){
				if($_GameInfos['GameMode'] != 3 || $pl['NbrLapsFinished'] >= $_GameInfos['LapsNbLaps'])
					$_players_finished++;
			}else if($pl['FinalTime'] == 0)
				$_players_giveup++;
		}
		...
		if($_fteams_scoresrule !== 'CPTime'){
			...
			foreach($_fteams_round as $ftid => &$fteamr){
				...
				if($prevrank == $fteamr['RoundRank']){
					...
					$averageteams[$ftid] = $fteamr['RoundScore'];

				}else{
					if(count($averageteams) > 1){
						// compute average score
						fteamsRoundAverageScores($averageteams);
					}
					$averageteams = array($ftid=>$fteamr['RoundScore']);
				}
	...
	foreach($_fteams_round as &$fteamr){
		...
		if($_fteams_ranksrule != 'CPTime'){
			...
			if($fteamr['RoundPoints'] > 0){
				$nbrank++;
				if($_fteams_drawrule == 'Average' && $prevrank > 0 && $prevpoints == $fteamr['RoundPoints'])
			...
			if($fteamr['RoundCPs'] > 0 && $fteamr['RoundTime'] > 0){
				$nbrank++;
				if($_fteams_drawrule == 'Average' && $prevrank > 0 && $prevcps == $fteamr['RoundCPs'] && $prevtime == $fteamr['RoundTime'])
					// same as prev : get prev rank
					$fteamr['RoundRank'] = $prevrank;
				else
					$fteamr['RoundRank'] = $rank;
...
// -----------------------------------
// set to fteams having the same round rank the average of their points
// used by fteamsComputePositions()
// -----------------------------------
function fteamsRoundAverageScores($averageteams){
	global $_fteams_round;
	$nb = count($averageteams);
	if($nb > 1){
		$sumscore = 0;
		foreach($averageteams as $ftid => $score){
			$sumscore += $score;
		}
		$average = (int)ceil($sumscore / $nb);
		foreach($averageteams as $ftid => $score){
			$_fteams_round[$ftid]['RoundScore'] = $average;
		}
	}
}
...
function playersEverysecond($event,$sec){
	...
	if($_StatusCode == 4 && $_GameInfos['GameMode'] != 1 && $_GameInfos['GameMode'] != 4){
		$playing = 0;
		$tmpspec = 0;
		foreach($_players as $login => &$pl){
	...
	if($_players_checkchange && $_GameInfos['GameMode'] != 1 && $_GameInfos['GameMode'] != 4 && $every <= 3){
...
function playersPlayerChat($event,$login,$message,$iscommand){
	global $_debug,$_players,$_ChatFloodRateMax,$_SystemInfo,$_remote_controller_chat_is_admin;
	if(!isset($_players[$login]['Active'])){
	...
	if($_players[$login]['Active'] && !$_players[$login]['Relayed'] && !$_players[$login]['ChatFloodRateIgnore'] &&
		 $_players[$login]['ChatFloodRate'] > $_ChatFloodRateMax*2 &&
		 (!verifyAdmin($login) || $_players[$login]['ChatFloodRate'] > $_ChatFloodRateMax*4)){
