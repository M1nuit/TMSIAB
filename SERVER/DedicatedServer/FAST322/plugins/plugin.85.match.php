...
function matchInit($event){
	global $_debug,$_match_mode,$_match_startit,$_match_config,$_match_conf,$match_file,$_match_map,$_match_map_rs,$_match_map_rswu,$_match_scores,$_match_scores_xml,$_match_scores_xml2,$_match_scores_bg_xml,$_match_scoretable_xml,$_match_scoretable_bg_xml,$_match_rounds_player_bonus,$_players_roundplayed_current,$_roundslimit_rule,$_match_show_freeplay,$_match_state_bg_xml,$_match_logo_xml,$_match_rounds,$_match_starttime,$_lastEverysecond;
	...
	$_match_scoretable_bg_xml = 
	sprintf('<frame posn="%0.2F %0.2F 10">'
					.'<quad sizen="52 5" posn="-1 0 0" style="Bgs1InRace" substyle="BgTitle3"/>'
					.'<label sizen="50 4" posn="26 -3 2" halign="center" valign="center2" style="TextCardRaceRank" '
					.'text="$wScores"/>'
					.'</frame>',
					-24,38);
	$_match_scoretable_xml = $_match_scoretable_bg_xml;
	$_match_logo_xml = '';
...
function matchShowScores($inchat=true){
	global $_debug,$_match_conf,$_players,$_match_map,$_match_map_rs,$_match_scores,$_GameInfos,$_ServerOptions,$_WarmUp,$_FWarmUp,$_players_roundplayed_current,$_players_round_current,$_match_scores_xml,$_match_scores_xml2,$_match_scores_bg_xml,$_match_scoretable_xml,$_match_scoretable_bg_xml,$_roundslimit_rule;
	...
	if($_match_map > 0){
		...
		foreach($_match_scores as &$pls){
			$lines++;
			$bcol = $pls['Bonus'] > 0 ? '$f00':'';
			$_match_scoretable_xml .=
			sprintf('<frame posn="%0.2F %0.2F 10">'
							.'<quad  sizen="52 7" posn="-1 1 0" style="Bgs1InRace" substyle="BgWindow1"/>'
							.'<label sizen="5 4" posn="2.7 -3 2" halign="center" valign="center2" style="TextRankingsBig" text="$ff4%d%s."/>'
							//.'<label sizen="5 4" posn="2.7 -3 2" halign="center" valign="center2" text="_" action="1"/>'
							.'<label sizen="34 4" posn="5.5 -3 2" valign="center2" style="TextRankingsBig" text="%s"/>'
							//.'<label sizen="34 4" posn="5.5 -3 2" valign="center2" text="_" action="1"/>'
							.'<label sizen="10 5" posn="45 -3.1 2" halign="center" valign="center2" style="TextValueBig" text=" $o%d "/>'
							//.'<label sizen="10 5" posn="45 -3.1 2" halign="center" valign="center2" text="_" action="1"/>'
							.'</frame>'
							,$posx,$posy,$pls['Rank'],$bcol,$pls['NickDraw'],
							((int)($pls['FullScore']+$pls['Bonus']+$pls['MapScore'])));
			$posy -= 8;
			if($lines >= 8)
				break;
		}
...
function matchPlayerConnect($event,$login){
	...
	matchScoreManialink($login);
	if($_StatusCode == 5 && isset($_players[$login]['ML']['Hide.scorepanel']) &&
		 $_players[$login]['ML']['Hide.scorepanel'] > 1)
		manialinksShowForce(''.$login,'match.scoretable',$_match_scoretable_xml);
...
function matchPlayerShowML($event,$login,$ShowML){
	global $_mldebug,$_players,$_StatusCode,$_match_scoretable_xml,$_match_map;
...
function matchBeginRace($event,$GameInfos){
	global $_debug,$_match_startit,$_match_map,$_match_map_rs,$_match_map_rswu,$_match_conf,$_match_scores,$_WarmUp,$_FWarmUp,$_players,$_match_roundmaxtime;
...
function matchEndRound($event,$Ranking,$ChallengeInfo,$GameInfos,$SpecialRestarting){
	global $_debug,$_players,$_match_map,$_match_map_rs,$_match_map_rswu,$_match_conf,$_match_scores,$_GameInfos,$_WarmUp,$_FWarmUp,$_match_scoretable_xml,$_match_rounds,$_match_starttime,$_lastEverysecond,$_players_round_current,$_fteams_round,$_fteams,$_FGameModes,$_FGameMode,$_currentTime;

	if($SpecialRestarting || $_match_map < 1 || $_WarmUp || $_FWarmUp > 0 || !isset($_match_conf['EndMatch']))
		return;
	if($_match_conf['MapRestartsWu'] > 0 && $_match_map_rswu < $_match_conf['MapRestartsWu'])
		return;
	console("matchEndRound:: MapRestartsWu: {$_match_map_rswu},{$_match_conf['MapRestartsWu']}");
	...
	// show scoretable to "screen" players
	foreach($_players as $login => &$pl){
		if($pl['Active'] && $pl['ML']['Hide.scorepanel'] > 1 && isset($pl['screen']) && $pl['screen']){
			//if($_debug>3) console("matchEndRound - $login - $_match_scoretable_xml");
			manialinksShowForce(''.$login,'match.scoretable',$_match_scoretable_xml);
		}
	}
...
function matchEndRace($event,$Ranking,$ChallengeInfo,$GameInfos,$continuecup,$warmup,$fwarmup){
	global $_debug,$_match_conf,$_players,$_match_map,$_match_map_rs,$_match_map_rswu,$_match_scores,$_GameInfos,$_WarmUp,$_FWarmUp,$_players_round_current,$_match_scoretable_xml,$_players_roundplayed_current,$_roundslimit_rule,$_EndMatchCondition,$_currentTime,$_players_round_time,$_FGameModes,$_FGameMode;

	// show scoretable to "screen" players
	foreach($_players as $login => &$pl){
		if($pl['Active'] && $pl['ML']['Hide.scorepanel'] > 1 && isset($pl['screen']) && $pl['screen']){
			//if($_debug>2) console("matchEndRound - $login - $_match_scoretable_xml");
			manialinksShowForce(''.$login,'match.scoretable',$_match_scoretable_xml);
		}
	}
	if($_match_map < 1)
	...
	if($_WarmUp || $_FWarmUp > 0 || count($_match_scores) <= 0){
		if(isset($_match_conf['EndMatch']) && $_match_conf['EndMatch']){
			if($_debug>1) console("matchEndRace:: End by no scores... match_map={$_match_map}, nbscores=".count($_match_scores).", WarmUp={$_WarmUp}, FWarmUp={$_FWarmUp}, EndMatch={$_match_conf['EndMatch']}, EndMatchCondition={$_EndMatchCondition}");
			matchEnd();
		}else{
			if($_debug>2) console("matchEndRace:: not end... match_map={$_match_map}, nbscores=".count($_match_scores).", WarmUp={$_WarmUp}, FWarmUp={$_FWarmUp}, EndMatch={$_match_conf['EndMatch']}, EndMatchCondition={$_EndMatchCondition}");
		}
		return;
	}
	...
	if($_debug>2) console("matchEndRace:: (state) match_map={$_match_map}, rswu={$_match_map_rswu}, rs={$_match_map_rs}, WarmUp={$_WarmUp}, FWarmUp={$_FWarmUp}, EndMatchCondition={$_EndMatchCondition}, {$scoremax} >= {$_GameInfos['RoundsPointsLimit']}, {$_players_roundplayed_current}>{$_roundslimit_rule} , ".($_currentTime - $_players_round_time));
	...
	if(isset($_FGameModes[$_FGameMode]['MatchEndRace']) && function_exists($_FGameModes[$_FGameMode]['MatchEndRace'])){
		...
		// -------------------------------------------------------------------------
		call_user_func($_FGameModes[$_FGameMode]['MatchEndRace'],$Ranking,$ChallengeInfo,$GameInfos,$continuecup,$warmup,$fwarmup,$scoremax,$timemin);
		...
		if($_match_conf['EndMatch'] || ($scoremax >= $_GameInfos['RoundsPointsLimit'] && $_GameInfos['RoundsPointsLimit'] > 0) ||
			...
			console("matchEndRace:(0): added full scores: ".print_r($_match_scores,true));
			
			// end of match
			if($_match_conf['EndMatch'] || $_match_map > $_match_conf['NumberOfMaps']){
				if($_debug>1) console("matchEndRace:: Rounds, all maps played... match_map={$_match_map}, nbscores=".count($_match_scores).", WarmUp={$_WarmUp}, FWarmUp={$_FWarmUp}, EndMatch={$_match_conf['EndMatch']}, EndMatchCondition={$_EndMatchCondition}");
				matchEnd();
			}
		...
		if($_match_conf['EndMatch'] || $_currentTime - $_players_round_time > $_GameInfos['TimeAttackLimit'] - 10000){
			...
			console("matchEndRace:(1): added full scores: ".print_r($_match_scores,true));
		...
		if($_match_conf['EndMatch'] || $_currentTime - $_players_round_time > $_GameInfos['TimeAttackLimit'] - 10000){
			...
			console("matchEndRace:(4): before add full scores: ".print_r($_match_scores,true));
			foreach($_match_scores as &$pls){
				if($_match_conf['GlobalScore']){
					$pls['FullScore'] += $pls['MapScore'];
					$pls['FullTime'] += $pls['BestTime'];
			...
			console("matchEndRace:(4): added full scores: ".print_r($_match_scores,true));

			if($_match_conf['MapRestarts'] > 1 && $_match_map_rs < $_match_conf['MapRestarts']){

				if($_debug>1) console("matchEndRace:: Stunts, map restart... match_map={$_match_map}, rswu={$_match_map_rswu}, rs={$_match_map_rs}, nbscores=".count($_match_scores).", WarmUp={$_WarmUp}, FWarmUp={$_FWarmUp}, EndMatch={$_match_conf['EndMatch']}, EndMatchCondition={$_EndMatchCondition}");
				$_match_map_rs++;
				addCall(null,'ChallengeRestart');
				...
				// end of match
				if($_match_conf['EndMatch'] || $_match_map > $_match_conf['NumberOfMaps']){
					if($_debug>1) console("matchEndRace:: Stunts, all maps played... match_map={$_match_map}, rswu={$_match_map_rswu}, rs={$_match_map_rs}, nbscores=".count($_match_scores).", WarmUp={$_WarmUp}, FWarmUp={$_FWarmUp}, EndMatch={$_match_conf['EndMatch']}, EndMatchCondition={$_EndMatchCondition}");
					matchEnd();
				}
...
function matchStart($login, $params, $startdelay=0){
	...
	if($_match_map >= 0){
		...
		if(!isset($_match_conf['MapScoresMode']) ||
			...
			console("MapScoresMode={$_match_conf['MapScoresMode']} ({$match_gamemode},{$match_fgamemode},{$fteams}), set right value in config !");
		}
...
function matchStop($msg=null){
	global $_debug,$_match_conf,$_ServerInfos,$_match_map,$_match_map_rswu,$_match_map_rs,$_players_round_current,$_match_scoretable_xml,$_match_scoretable_bg_xml,$_match_OldGameInfos,$_NextGameInfos,$_StatusCode;
...
function chat_match($author, $login, $params1, $params){
	global $_debug,$_match_conf,$_match_map,$_match_scores,$_is_relay,$_NextGameInfos,$_GameInfos,$_ChallengeList,$_players,$_mapinfo_default,$_scorepanel_hide,$_match_scoretable_xml,$_MapFalsestart;
	...
	if(isset($params[0]) && ($params[0] == 'start' || $params[0] == 'on')){
		...
		if(!verifyAdmin($login)){
			...
			matchStop($author.localeText(null,'interact')." (admin) stopped the match !");
		...
		if(!verifyAdmin($login)){
			...
			$_match_conf['EndMatch'] = true;
			matchEnd($author.localeText(null,'interact')." (admin) finished the match !");
		}
		...
		if(isset($params[1]) && $params[1] == 'on'){
			...
			//console("matchEndRound - $login - $_match_scoretable_xml");
			if(isset($_match_conf['NumberOfMaps']) && $_match_conf['NumberOfMaps'] > 0)
				manialinksShowForce(''.$login,'match.scoretable',$_match_scoretable_xml);
