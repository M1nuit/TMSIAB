					...
					if(isset($tmdb['Challenge']['Records'])){
						foreach($tmdb['Challenge']['Records'] as &$rec){
							if($rec['Login']==$login){
								...
								$rec['NickDraw'] = htmlspecialchars(tm_substr(stripColors($rec['NickName']),0,13));
								break;
							}
						}
					}
...
function databaseBeginChallenge_DB($response,$tm_db_n,$ChallengeInfo){
	global $_debug,$_tm_db,$_NumberOfChecks,$_ServerOptions,$_BestChecks,$_BestPlayersChecks,$_BestChecksName,$_players;
...
function databaseEndRace($event,$Ranking,$ChallengeInfo,$GameInfos){
	...
	}elseif($GameInfos['GameMode'] == 0 && (!isset($ChallengeInfo['LapRace']) || $ChallengeInfo['LapRace']) &&
	...
	for($tm_db_n=0; $tm_db_n < count($_tm_db); $tm_db_n++){
		...
		if(isset($tmdb['XmlrpcDB']) && !$tmdb['XmlrpcDB']->isBad()){
			...
			if(!isset($tmdb['Challenge']['Uid'])){
				...
				for($j=0;($j< count($records)) && ($time>=$records[$j]['Best']) && ($records[$j]['Login']!=$login);$j++)
					;
				...
				$old_rank = -1;
				$old_best = -1;
				if($j<count($records) && $records[$j]['Login']==$login){
					$old_rank = $j+1;
				...
				if(($j>=count($records) && $j<$max) || ($j<count($records) && $time<$records[$j]['Best'])){
					...
					if(count($records)>0 && $j<count($records) && $records[$j]['Login']!=$login){
						// find an old record of himself or go to end
						for($k=$j+1;$k<count($records) && $records[$k]['Login']!=$login && $k<$max;$k++)
							;
						if($k<count($records) && $records[$k]['Login']==$login){
							$old_rank = $k+1;
					...
					$records[$j] = array('Login'=>$login,'NickName'=>tm_substr($_players[$login]['NickName']),'Best'=>$time,'Rank'=>$j+1,'NewBest'=>true,'NickDraw'=>$_players[$login]['NickDraw'],'Checks'=>$checkpts);
...
function database_mkPlayerInfo($login){
  ...
  if(!is_string($login))
	...
	if(isset($_players[$login]['Login']) &&  $_players[$login]['Login']==$login &&
		...
		return array('Login'=>$login,'NickName'=>$nickname,'Nation'=>$nation,'TeamName'=>$teamname,
								 'TeamId'=>$teamid,'IsSpec'=>$isspec,'Ranking'=>$ranking,'IsOff'=>$isoff);
		// array('Login','Nation','TeamName','TeamId','IsSpec','Ranking','IsOff')
	}
	return false;
}
