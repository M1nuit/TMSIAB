...
namespace ManiaLivePlugins\MLEPP\Widgets\Widgets\Views\Defaults\Sidebar;
...
class Standard extends TypeView {
    ...
    public function setSettings($set){
		...
		if($mlepp->gameVersion->name == "ManiaPlanet"){
			$this->bg_title = new \ManiaLib\Gui\Elements\Bgs1($this->settings->width, 128);
			$this->bg_title->setSubStyle("BgTitleGlow");
		}else{
			$this->bg_title = new \ManiaLib\Gui\Elements\Bgs1InRace($this->settings->width, 128);
