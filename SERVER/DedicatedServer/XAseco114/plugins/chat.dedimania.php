...
function chat_dedirecs($aseco, $command) {
	...
	if ($aseco->server->getGame() == 'TMN') {
		$head = 'Current TOP ' . $dedi_db['MaxRecords'] . ' Dedimania Records:' . LF;
		$msg = '';
		$lines = 0;
		$player->msgs = array();
		$player->msgs[0] = 1;
	...
	// display ManiaLink window for TMF
		...
		$head = 'Current TOP ' . $dedi_db['MaxRecords'] . ' Dedimania Records:';
		$msg = array();
		$lines = 0;
		$player->msgs = array();
...
function chat_dedicps($aseco, $command) {
	...
	if ($aseco->settings['display_checkpoints']) {
		if (isset($checkpoints[$login]) && $checkpoints[$login]->loclrec != -1) {
			...
			elseif (is_numeric($param) && $param > 0 && $param <= $dedi_db['MaxRecords']) {
...
function chat_dedisectms($aseco, $command) {
	global $dedi_db;
	...
	// display popup window for TMN
	if ($aseco->server->getGame() == 'TMN') {
		$head = 'Current TOP ' . $dedi_db['MaxRecords'] . ' Dedimania Sector Times (' . $cpscnt . '):' . LF;
		$cpsmax = 9;
		$msg = '';
		$lines = 0;
		$player->msgs = array();
	...
	// display ManiaLink window for TMF
		...
		$head = 'Current TOP ' . $aseco->server->records->max . ' Dedimania Sector Times (' . $cpscnt . '):';
		$cpsmax = 12;
		// compute widths
	...
	// show chat message for TMO & TMS
		...
		$msg = $aseco->formatColors('{#server}> {#error}No Dedimania sector times available');
		$aseco->client->query('ChatSendServerMessageToLogin', $msg, $login);
	}
}  // chat_dedisectms
?>
