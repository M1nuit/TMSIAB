...
function ta_expire_votes($aseco, $data) {
	...
	if ($aseco->server->gameinfo->mode == 0 ||
	    ...
	    $aseco->server->gameinfo->mode == 5) return;
...
function chat_ladder($aseco, $command) {
	...
	if ($aseco->server->gameinfo->mode == 0 && $r_points_limits) {
	...
	elseif ($aseco->server->gameinfo->mode == 1 && $ta_time_limits) {
