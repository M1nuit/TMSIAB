	...
	if (!empty($jukebox)) {
		...
		if ($next['tmx']) {
			...
			if (!$rtn) {
				...
				if ($aseco->server->getGame() == 'TMF' &&
				    $aseco->client->getErrorMessage() == "The track doesn't match the server packmask.") {
					$message = formatText($rasp->messages['JUKEBOX_PACKMASK'][0],
					                      $next['Env'], stripColors($next['Name']));
					if ($jukebox_in_window && function_exists('send_window_message'))
						send_window_message($aseco, $message, true);
					else
						$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
		...
		if ($feature_karma) {
			...
			$help[] = array('...', '{#black}nogold',
			                'Shows tracks you didn\'t beat gold ' .
			                 ($aseco->server->gameinfo->mode == 4 ? 'score on' : 'time on'));
