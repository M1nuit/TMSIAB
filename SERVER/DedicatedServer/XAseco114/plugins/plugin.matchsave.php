...
function chat_teamname($aseco, $command) {
	...
	if ($teamname == 'help' || trim($teamname) == '') {
		...
		if ($aseco->server->getGame() == 'TMN') {
			if ($player->teamname) {
				$msg = 'Your currently belong to team '.$MatchSettings['col_window_highlite_team'].$player->teamname.'$z'.LF;
			...
			$msg = array();
			if ($player->teamname) {
				$header = 'Your currently belong to team '.$MatchSettings['col_window_highlite_team'].$player->teamname;
