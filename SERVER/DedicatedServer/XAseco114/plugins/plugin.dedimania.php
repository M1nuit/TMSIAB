		...
		if (isset($config['DEDIMANIA']['DATABASE']) && is_array($config['DEDIMANIA']['DATABASE']) &&
			...
			if (isset($dbdata['URL'][0])) {
				if (!is_array($dbdata['URL'][0]))
					$dedi_db['Url'] = array($dbdata['URL'][0]);
				else {
					$dedi_db['Url'] = array();
				...
				if (isset($dbdata['MAX_RECS'][0]))
					$dedi_db['MaxRecords'] = intval($dbdata['MAX_RECS'][0]);
				else
					$dedi_db['MaxRecords'] = $dedi_db_defaults['MaxRecords'];
...
function dedimania_connect($aseco) {
	...
	if (!isset($dedi_db['XmlrpcDB']) &&
		...
		if (!isset($dedi_db['Urltest']))
			$urltest = 0; // rand(0, count($dedi_db['Url']) - 2);
		else
			$urltest = 0; // ($dedi_db['Urltest'] + 1) % count($dedi_db['Url']);
		$url = $dedi_db['Url'][$urltest];

		$aseco->console("* Dataserver connection on " . $dedi_db['Name'] . ' ...');
		$aseco->console('* Try connection on ' . $url . ' ...');

		// establish Dedimania connection and login (always on last URL)
		$xmlrpcdb = new XmlrpcDB($dedi_webaccess, $dedi_db['Url'][count($dedi_db['Url']) - 1],
		...
		elseif (isset($response['Data']['params']['Status']) && $response['Data']['params']['Status']) {
			...
			if (isset($response['Data']['errors']) && strlen($response['Data']['errors']) > 0)
				$aseco->console_text('  !!!' . CRLF . '  !!! ...with an authentication warning: ' . $response['Data']['errors']);
		}
		elseif (isset($response['Data']['errors'])) {
			$aseco->console_text('  !!!' . CRLF . '  !!! Connection Error !!! (' . $response['Headers']['server'][0] . ')' . CRLF
			                     . $response['Data']['errors'] . CRLF . '  !!!');
		}
		elseif (!isset($response['Code'])) {
			$aseco->console_text("  !!!' . CRLF '  !!! Error no database response (' . $url . ')' . CRLF . '  !!!");
		}
		else {
...
function dedimania_newchallenge($aseco, $challenge) {
	...
	if ($aseco->server->getGame() == 'TMF' && $challenge->laprace &&
	    ($aseco->server->gameinfo->mode == 0 ||
	     $aseco->server->gameinfo->mode == 2 ||
	     $aseco->server->gameinfo->mode == 5)) {
		if ($challenge->forcedlaps == 0)
			$dedi_db['TrackValid'] = true;
		else
			if ($dedi_debug > 0)
				$aseco->console_text('dedimania_newchallenge - RoundForcedLaps != 0: records ignored');
	}
	// check for minimum author time and no Stunts mode
	elseif ($challenge->authortime >= $dedi_minauth &&
	        $aseco->server->gameinfo->mode != 4)
		$dedi_db['TrackValid'] = true;
	else
		if ($dedi_debug > 0)
			$aseco->console_text('dedimania_newchallenge - Author time < ' . ($dedi_minauth / 1000) . 's: records ignored');
...
function dedimania_playerfinish($aseco, $finish_item) {
	...
	if (!$dedi_db['RecsValid'] || $aseco->server->gameinfo->mode == 4) return;
	...
	if ($dedi_debug > 0)
		if ($aseco->server->gameinfo->mode != 4 &&
		    ($finish_item->score != $checkpoints[$login]->curr_fin ||
		     $finish_item->score != end($checkpoints[$login]->curr_cps)))
			$aseco->console('dedimania_playerfinish - inconsistent finish/checks!  score: ' . $finish_item->score . '  checks:' . CRLF . print_r($checkpoints[$login], true));

	// point to master records list
	$dedi_recs = &$dedi_db['Challenge']['Records'];

	// go through all records
	for ($i = 0; $i < $dedi_db['MaxRecords']; $i++) {
		// check if no record, or player's time/score is better
		if (!isset($dedi_recs[$i]) || ($aseco->server->gameinfo->mode == 4 ?
			...
			if ($cur_rank != -1) {  // player has a record in topXX already
				...
				if (count($dedi_recs) >= $dedi_db['MaxRecords'])
					array_pop($dedi_recs);
