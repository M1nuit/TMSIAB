...
function chat_admin($aseco, $command) {
	 ...
	 */
		...
		if ($command['params'][1] != '' && is_numeric($command['params'][1]) && $command['params'][1] != 0) {
			...
			// show chat message
			$message = formatText('{#server}> {#admin}The track history contains {#highlite}{3}{#admin} tracks',
			...
			if (is_numeric($command['params'][$id]) && $command['params'][$id] >= 0) {
				...
				if ($file === false || $file == -1) {
					...
					if (!$rtn) {
						...
						$message = formatText('{#server}> {#error}Error adding track {#highlite}$i {1} {#error}!',
						                      $partialdir);
						$aseco->client->query('ChatSendServerMessageToLogin', $aseco->formatColors($message), $login);
					} else {
	 ...
	 */
		...
		if ($command['params'][1] != '') {
			$sepchar = substr($aseco->server->trackdir, -1, 1);
			$partialdir = 'Challenges' . $sepchar . 'Downloaded' . $sepchar . $command['params'][1];
	 ...
	 */
		...
		if (file_exists($aseco->server->trackdir . 'MatchSettings/' . $filename)) {
			...
			if (!$rtn) {
				...
				$message = '{#server}> {#highlite}' . $filename . '{#admin} read with {#highlite}' . $cnt . '{#admin} tracks';
			...
			if ($autosave_matchsettings) {
				if (file_exists($aseco->server->trackdir . 'MatchSettings/' . $autosave_matchsettings)) {
					...
					if (!$rtn) {
						...
						$message = formatText('{#server}>> {#admin}{1}$z$s {#highlite}{2}$z$s{#admin} shuffled track list with {#highlite}{3}{#admin} tracks!',
						                      $chattitle, $admin->nickname, $cnt);
						$aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));
						return;
					}
				} else {
	 ...
	 */
		...
		if ($aseco->server->getGame() == 'TMF') {
			if (strtolower($command['params'][1]) == 'off') {
				...
				// load default style
				if ($style = $aseco->xml_parser->parseXml($style_file)) {
