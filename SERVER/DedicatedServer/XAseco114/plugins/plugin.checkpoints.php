	...
	if ($aseco->settings['display_checkpoints']) {
		...
		if ($aseco->server->getGame() == 'TMF') {
			if ($checkpoints[$login]->loclrec == -1) {
				...
				if (!$player->isspectator && $aseco->server->gameinfo->mode != 4 && !$aseco->warmup_phase) {
					if ($aseco->settings['enable_cpsspec'] && !empty($checkpoints[$login]->speccers))
...
function chat_sectms($aseco, $command) {
	...
	// find sector count from first record with sector times
	$cpscnt = '?';
	for ($i = 0; $i < $total; $i++) {
	...
	if ($aseco->server->getGame() == 'TMN') {
		$head = 'Current TOP ' . $aseco->server->records->max . ' Local Sector Times (' . $cpscnt . '):' . LF;
		$cpsmax = 9;
		$msg = '';
		$lines = 0;
		$player->msgs = array();
		...
		for ($i = 0; $i < $total; $i++) {
			...
			// append up to $cpsmax sector times
			if (!empty($cur_record->checks)) {
				$j = 1;
				$pr = 0;
				$msg .= '$n';
	...
	// display ManiaLink window for TMF
		...
		$head = 'Current TOP ' . $aseco->server->records->max . ' Local Sector Times (' . $cpscnt . '):';
		$cpsmax = 12;
		// compute widths
...
function clear_curr_cp($aseco) {
	...
	if ($aseco->server->gameinfo->mode == 4 || $aseco->warmup_phase) return;
...
function store_checkp($aseco, $checkpt) {
	...
	if ($aseco->server->gameinfo->mode != 3) {
		...
		if ($aseco->server->gameinfo->mode == 1 && $checkpt[4] == 0)
			$checkpoints[$login]->curr_cps = array();
		...
		if ($checkpt[3] * $laps_cpcount != $checkpt[4] + 1) {
			...
			if ($checkpoints[$login]->loclrec != -1 && $checkpt[4] > $laps_cpcount) {
				// check for improvement
				if ($diff < 0) {
					$diff = abs($diff);
					$sign = '$00f-';  // blue
...
function store_finish($aseco, $finish_item) {
	...
	if ($aseco->server->gameinfo->mode == 3 ||
	    $aseco->server->gameinfo->mode == 4) return;
	...
	if ($finish_item->score > 0) {
		...
		if ($aseco->server->getGame() == 'TMF' && $aseco->server->gameinfo->mode != 1 &&
	...
	elseif ($aseco->server->getGame() == 'TMF' && $aseco->server->gameinfo->mode == 1 &&
