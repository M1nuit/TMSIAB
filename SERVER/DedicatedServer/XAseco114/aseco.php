 ...
 *
 * Re-authored & copyright May 2007 - Aug 2011 by Xymph <tmn@gamers.org>
 */
...
class Aseco {
	...
	function readLists() {
		...
		if ($lists = $this->xml_parser->parseXml($adminops_file)) {
		...
		if (!empty($calls)) {
			while ($call = array_shift($calls)) {
				switch ($call[0]) {
					...
					case 'TrackMania.PlayerCheckpoint':  // [0]=PlayerUid, [1]=Login, [2]=Time, [3]=Score, [4]=CheckpointIndex
						if (!$this->server->isrelay)
	...
	function newChallenge($challenge) {
		...
		if ($this->server->getGame() == 'TMF' && $challenge_item->laprace &&
		    ($this->server->gameinfo->mode == 0 ||
		     $this->server->gameinfo->mode == 2 ||
		     $this->server->gameinfo->mode == 5)) {
			$challenge_item->forcedlaps = $this->server->gameinfo->forcedlaps;
		}

		// obtain challenge's GBX data, TMX info & records
		$challenge_item->gbx = new GBXChallengeFetcher($this->server->trackdir . $challenge_item->filename, true);
		// check for XML parser error
		if (is_string($challenge_item->gbx->parsedxml))
			trigger_error($challenge_item->gbx->parsedxml, E_USER_WARNING);
...
disabled */
		...
		if (!$this->server->isrelay) {
			...
			if ($cur_record !== false && $cur_record->score > 0) {
				$score = ($this->server->gameinfo->mode == 4 ?
				...
				if ($this->server->gameinfo->mode == 4)
					$score = '  ---';
				else
					$score = '   --.--';
	...
	function isMasterAdmin($player) {
		...
		if (isset($this->masteradmin_list['TMLOGIN']))
