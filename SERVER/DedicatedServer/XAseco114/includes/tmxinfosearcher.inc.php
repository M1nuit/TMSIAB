 ...
 */
class TMXInfoSearcher implements Iterator {
	...
	public function current() {
		$track = new TMXInfo($this->section, $this->prefix, current($this->tracks));
		return $track;
	}
	public function next() {
		$track = new TMXInfo($this->section, $this->prefix, next($this->tracks));
		return $track;
	}
	public function key() {
		$key = key($this->tracks);
	...
	private function getRecent() {
		...
		$file = $this->get_file('http://' . $this->prefix . '.tm-exchange.com/apiget.aspx?action=apirecent');
	...
	private function getList($name, $author, $env) {
		...
		// compile search URL
		$searchurl = 'http://' . $this->prefix . '.tm-exchange.com/apiget.aspx?action=apisearch';
		if ($name != '')
			$searchurl .= '&track=' . $name;
		if ($author != '')
		...
		while ($page < $maxpage && !$done) {
			$file = $this->get_file($searchurl . $page);
			if ($file === false || $file == -1)
				return array();
