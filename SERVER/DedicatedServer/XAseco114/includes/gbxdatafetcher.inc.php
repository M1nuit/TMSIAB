...
/**
 * GBXDataFetcher - Fetch GBX challenge/replay data for TrackMania tracks
...
class GBXChallengeFetcher {
	...
	public $filename, $parsexml, $tnimage,
	       $uid, $version, $name, $author, $type, $envir, $mood,
	       $pub, $authortm, $goldtm, $silvertm, $bronzetm,
	       $coppers, $multi, $unknown, $unknown2, $ascore, $password,
	       $xml, $parsedxml, $xmlver, $exever, $nblaps, $songfile, $songurl,
	       $modname, $modfile, $modurl, $thumbnail, $comment;
	...
	private function getData() {
		...
		// get GBX version: 2/3 = TM/TMPowerUp, 4 = TMO(LevelUp)/TMS/TMN, 5 = TMU/TMF
		...
		if ($this->version >= 3) {
			$data = fread($handle, 4);
			$r = unpack('Vcoppers', $data);
			$this->coppers = $r['coppers'];
		}
		...
		fseek($handle, 0x01, SEEK_CUR);  // almost always 08
...
class GBXReplayFetcher {
	...
	public $filename, $parsexml,
	       ...
	       $xml, $parsedxml, $xmlver, $exever, $respawns, $stuntscore, $validable,
	       $cpscur, $cpslap;
